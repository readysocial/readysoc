...
const steps: { number: string; title: string; description: string; image?: string }[] = [
  ...
  function Hexagon({ children }: { children: React.ReactNode }) {
    return (
      <div className="w-14 h-14 flex items-center justify-center" style={{ clipPath: 'polygon(25% 6.7%, 75% 6.7%, 100% 50%, 75% 93.3%, 25% 93.3%, 0% 50%)', background: '#38BDF8' }}>
  ...
  export default function HowItWorks() {
    return (
      <section className="relative py-24 bg-white overflow-x-clip overflow-y-visible">
        {/* Large semicircle top right, aligned with heading */}
        <div className="absolute top-[60px] right-0 w-[378px] h-[378px] z-0 overflow-visible pointer-events-none">
          <div className="w-full h-full bg-[#5CC3EB] rounded-full relative" style={{transform: 'translateX(50%)'}}>
            {/* Medium circle inside semicircle */}
            <div className="absolute top-[114px] left-[-100px] w-[100px] h-[100px] bg-[#5CC3EB] rounded-full" />
            {/* Small circle inside semicircle */}
            <div className="absolute top-[16px] left-[-60px] w-[50px] h-[50px] bg-[#5CC3EB] rounded-full" />
          </div>
        </div>
        
        <div className="container mx-auto max-w-[1280px] px-6 md:px-8 relative z-10">
          <h2 className="text-4xl md:text-5xl font-bold text-center mb-24 text-black">
            How It Works
          </h2>
                  ...
                  {step.number === "02" || step.number === "04" ? (
                    <>
                      ...
                      <div className="flex flex-col items-start md:pl-0 relative z-10">
                        ...
                        <h3 className="text-2xl font-bold mb-4 text-black w-full text-left">{step.title}</h3>
                        <p className="text-black text-lg leading-relaxed w-full max-w-md text-left">{step.description}</p>
                      </div>
                      ...
                      <div className="flex flex-col items-center relative justify-center h-full">
                        {step.image && (
                          <Image 
                            ...
                            width={220} 
                            height={420} 
                            className="relative z-10 w-auto h-[340px] md:h-[420px] object-contain drop-shadow-lg" 
                          />
                        )}
                        {step.number === "04" && (
